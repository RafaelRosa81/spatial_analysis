...
This toolkit contains three pipelines:

- `raster_diff`: align two rasters, compute `dz`, and generate reports.
- `polygon_mosaic`: replace or blend raster values within a polygon.
- `sample_points_from_raster_value_range`: sample points from raster values
  inside a numeric range.

## Raster diff pipeline
...
secondary raster, with optional vertical adjustment and border blending. It
shares the same alignment logic as the raster diff pipeline, but adds polygon
masking and blending steps.

### Polygon mosaic vertical adjustment

An optional vertical adjustment applies a constant offset to raster2. The offset
is the **median** of the overlap `dz` values when:
...
- **Polygon masking:** the polygon footprint is rasterized to create a binary
  mask. Outside the polygon is treated as no contribution from raster2.
...
## Sample points from raster value range pipeline

This pipeline filters raster pixels within a numeric range and then samples
point locations from those pixels.
...
flowchart LR
  A[Input raster]
  B[Filter pixels in value range]
  C{Sampling method}
  D[Random selection]
  E[Regular spacing]
  F[Outputs: CSV and/or GeoPackage]
...
- **Inclusive range:** pixels are retained when `value_min <= value <= value_max`.
- **Random sampling:** uses `n_points` and optional `seed` for reproducibility.
- **Regular sampling:** uses `spacing` in map units; use a projected CRS if you
  need meter-based spacing.
